...
class Peer {
  ...
  async _setName(req) {
    ...
    try {
      console.log(`Trying to obtain a Korean name.`);
      const response = await fetch('https://nickname.hwanmoo.kr/?format=json&count=1&max_length=8');
      const body = await response.json();
      displayName = body.words[0];
    } catch (e) {
      console.error(`Unable to obtain Korean name. Using fallback name generator. The error was ${e}`);
